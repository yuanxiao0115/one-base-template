...
## 公文表单引擎包化（DocumentFormManagement + document-form-engine）

- 结论：
  - `@one-base-template/document-form-engine` 已以独立 package 落地，admin 通过 `DocumentFormManagement` 模块薄接入。
  - 新包、admin 定向、docs 与根级 `typecheck/lint/build` 已完成验证。
  - 根级 `pnpm verify` 仍被既有命名门禁阻塞，阻塞点不在本次改动文件内。
- 关键产物：
