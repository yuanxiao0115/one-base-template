...
- `apps/admin-lite` 与独立项目的基础 CSS 文件没有丢失。
- 真实差异是 Tailwind v4 扫描源：仓内应用扫描 `packages/ui/src`，独立项目必须扫描 `node_modules/@one-base-template/ui/dist`。
- 模板已补齐独立项目扫描源，避免 `@one-base-template/ui` 中的布局/间距工具类未生成。
...
- `tag` 包的完整样式产物存在于 `dist/style.css`。
- 原 `@one-base-template/tag/style` 指向 `dist/styles/global.scss`，只带变量入口，独立项目最终 CSS 缺少 `.tags-view`、`.context-menu`、`.dropdown-menu` 等组件选择器。
- 样式入口已改为 `dist/style.css`，CLI 验证已覆盖最终 CSS 产物。
