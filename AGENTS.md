...
- **规则来源不等于规则作用域**：规则即使最初在某个模块（如 `UserManagement`）中沉淀，只要后续验证适用于同层其他模块或整个 admin / 全仓，就必须提升到对应更高作用域，禁止继续挂在原模块语境下误导后续推广范围。
...
- **禁止过度防御性映射**：当字段契约已经明确时，禁止在 `form.ts` / `mapper` / `adapter` 里堆叠 `String(...)`、`Number(...)`、`value == null ? '' : ...`、`unknown -> helper -> helper` 这类“为了防御而防御”的归一化写法；优先按当前类型契约直接赋值，仅在真实存在跨边界协议转换或后端脏数据证据时，才允许在单一边界做一次必要转换。
- **可维护性第一**：禁止“为了架构好看而解耦”或“为了封装而封装”；设计与实现均以长期维护成本最低为准。
