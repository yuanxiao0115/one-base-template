...
describe('manual-chunks admin-lite feature chunk 策略', () => {
  ...
  it('应保持日志管理页面继续进入 feature chunk', () => {
    expect(manualChunks('/repo/apps/admin-lite/src/modules/LogManagement/login-log/list.vue')).toBe(
      'admin-lite-log-management'
    );
  });
});
...
describe('admin preload resolver', () => {
  ...
  it('LoginPage 不应预加载 app-shell/wangeditor 等重依赖', () => {
    const deps = [
      'assets/admin-lite-app-shell-xxx.js',
  ...
  it('admin-lite-app-shell 不应预加载 portal/vxe/element-plus 等重依赖', () => {
    const deps = [
    ...
    expect(
      preloadResolver('assets/admin-lite-app-shell-abc.js', deps, {
        hostId: 'assets/admin-lite-app-shell-abc.js',
        hostType: 'js'
      })
...
describe('admin preload map prune', () => {
  it('应重写 admin-lite-app-shell 的 built preload map，过滤重依赖前缀', () => {
    const source =
    ...
    const nextSource = pruneBuiltChunkPreloadMaps(source, 'assets/admin-lite-app-shell-abc.js', {
      appName: 'admin-lite'
    });
