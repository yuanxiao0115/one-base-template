...
- 门户管理模块目录名固定为 `PortalManagement`，但模块契约 `moduleMeta.id` 固定为 `portal-management`；管理侧路由路径固定为：`/portal/setting`、`/portal/design`、`/portal/page/edit`、`/portal/preview`，禁止继续为该模块配置路由 alias（含路由 `alias` 属性与 `compat.routeAliases`）。
...
- `PortalManagement/designPage/components/portal-template/PortalShellSettingsDialog.vue` 作为门户级页眉页脚配置容器必须使用 `ObCrudContainer`（`container="drawer"`）承载，抽屉宽度固定 `400`；禁止回退为 `el-dialog` 直连编排。
