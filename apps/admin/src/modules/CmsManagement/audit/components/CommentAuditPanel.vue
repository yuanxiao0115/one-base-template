...
<template>
  ...
  >
    <template #buttons>
      <el-button :icon="Delete" :disabled="selectedRows.length === 0" @click="batchDeleteComments">批量删除</el-button>
    </template>
      ...
      >
        ...
        <template #operation="{ row, size: actionSize }">
          <ObActionButtons>
            ...
            <el-button link type="primary" :size="actionSize" @click="() => openCommentDialog(row as CommentAuditRecord, 'detail')">
              详情
            </el-button>
            <el-button link type="danger" :size="actionSize" @click="() => deleteComment(row as CommentAuditRecord)">
              删除
            </el-button>
          </ObActionButtons>
        </template>
    ...
    <template #drawer>
      <el-form ref="searchRef" :model="searchForm" label-position="top" class="comment-audit-panel__search-form">
  ...
  >
    <div v-loading="dialogLoading" class="comment-audit-panel__dialog-body">
      <el-empty v-if="!(currentCommentDetail || dialogLoading)" description="未获取到评论详情" :image-size="88" />

      <template v-else-if="currentCommentDetail">
        <el-descriptions :column="2" border>
          <el-descriptions-item label="文章标题" :span="2">{{ currentCommentDetail.articleTitle || "-" }}</el-descriptions-item>
          <el-descriptions-item label="评论人">{{ currentCommentDetail.commentatorName || "-" }}</el-descriptions-item>
          <el-descriptions-item label="评论时间">{{ currentCommentDetail.commentTime || "-" }}</el-descriptions-item>
        ...
        <section class="comment-audit-panel__dialog-section">
          ...
          <div class="comment-audit-panel__comment-content">{{ currentCommentDetail.commentContent || "-" }}</div>
        </section>

        <el-form v-if="isReviewMode" ref="reviewFormRef" :model="reviewForm" :rules="reviewFormRules" label-position="top">
    ...
    <template #footer>
      <div class="comment-audit-panel__dialog-footer">
        <template v-if="isReviewMode">
          ...
          <el-button type="danger" :loading="dialogSubmitting" @click="submitCommentReview(2)">驳回</el-button>
          <el-button type="success" :loading="dialogSubmitting" @click="submitCommentReview(1)">通过</el-button>
        </template>
