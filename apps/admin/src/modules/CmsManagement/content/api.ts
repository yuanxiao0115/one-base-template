...
export const contentApi = {
  ...
  uploadResource: async (file: File, options?: UploadRequestOptions) => {
    ...
    const response = await obHttp().post<ApiResponse<UploadResourceResult>>(
  ...
  uploadAttachment: async (
    ...
    const response = await obHttp().post<ApiResponse<UploadAttachmentResult>>(
      '/cmict/file/upload-file',
      {
        data: formData,
        $isUpload: true,
        onUploadProgress: options?.onProgress
