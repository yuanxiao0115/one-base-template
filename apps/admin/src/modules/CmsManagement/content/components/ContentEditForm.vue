...
function normalizeAttachmentList(
  ...
  (attachments || []).forEach((item, index) => {
    ...
    normalizedList.push({
      attachmentName:
        String(item.attachmentName || '').trim() || buildAttachmentName(attachmentUrl, index),
      attachmentUrl
    });
  });
...
function findAttachmentIndex(
  attachments: ContentAttachmentFormItem[],
  target: UploadUserFile
): number {
  if (target.url) {
...
async function uploadEditorResource(payload: {
  file: File;
  type: 'image' | 'video';
}): Promise<string> {
...
const attachmentPreviewList = computed(() =>
  normalizeAttachmentList(model.value.cmsArticleAttachmentList)
);
        ...
        <div class="content-form-section content-form-section--meta">
          <div class="content-form-meta-grid">
            <el-form-item
              label="标题"
              prop="articleTitle"
              class="content-form-item content-form-item--full"
            >
              <el-input
            ...
            <el-form-item
              label="文章类型"
              prop="articleType"
              class="content-form-item content-form-item--half"
            >
            ...
            >
              <el-input
                v-model.trim="model.articleAuthorName"
                maxlength="50"
                show-word-limit
                placeholder="请输入作者"
              />
                ...
                >
                  ...
                  <template #tip>
                    <div class="cover-upload-tip">
                      支持 {{ coverAllowedExtensions.join('、') }} 格式，单张不超过
                      {{ coverMaxSizeMb }}MB，仅可上传 1 张
                    </div>
                  </template>
                </el-upload>
                <p v-if="!props.disabled" class="cover-upload-hint">
                  建议比例 16:9，获得更好的列表展示效果
                </p>
              </div>
            </el-form-item>
            ...
            <template v-if="isRepost">
              ...
              >
                <el-input
                  v-model.trim="model.articleSource"
                  maxlength="300"
                  show-word-limit
                  placeholder="请输入转载来源"
                />
              ...
              >
                ...
                <template #tip>
                  <div class="attachment-tip">
                    支持常见文档、压缩包、媒体文件，单文件不超过 {{ attachmentMaxSizeMb }}MB
                  </div>
                </template>
              </el-upload>
            </div>
