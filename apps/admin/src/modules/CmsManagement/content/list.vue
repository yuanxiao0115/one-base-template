<script setup lang="ts">
    ...
    table: {
      ...
      remove: {
        ...
        beforeDelete: async (row: ContentRecord) => {
          ...
          await obConfirm.warn(`是否确认删除标题为「${row.articleTitle || "-"}」的内容？`, "删除确认");
        },
        isCancelled: (error) =>
          error === "cancel" || error === "close" || (error instanceof Error && error.message === BLOCK_DELETE_REASON),
        onSuccess: () => {
    ...
    editor: {
      ...
      save: {
        ...
        request: async ({ mode, payload }) => {
          const response = mode === "create" ? await contentApi.add(payload) : await contentApi.update(payload);
          if (response.code !== 200) {
  ...
  function handleCrudError(error: unknown, context: CrudErrorContext<ContentRecord>) {
    const fallback =
      context.stage === "beforeOpen" ? "打开弹窗失败" : context.stage === "loadDetail" ? "加载详情失败" : "保存内容失败";
      ...
      <template #default="{ size, dynamicColumns }">
        ...
        >
          <template #reviewStatus="{ row }">
            <el-tag :type="getReviewStatusTagType(row.reviewStatus)">{{ getReviewStatusText(row.reviewStatus) }}</el-tag>
          </template>
