...
function toSysLogPageData(data: unknown): SysLogPageData {
  ...
  const records = extractList(payload).map((item) => toSysLogRecord(toRecord(item) as SysLogRawRecord));
...
export const sysLogApi = {
  ...
  detail: async (params: { id: string }) =>
    getHttpClient()
      .get<BizResponse<SysLogRecord>>("/cmict/logstore/sys-log/detail", {
        params,
      })
      .then((response) => ({
        ...response,
