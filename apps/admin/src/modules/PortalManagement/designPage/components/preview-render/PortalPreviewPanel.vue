<script setup lang="ts">
  ...
  function setRuntimeViewport(width: unknown, height: unknown) {
    const nextWidth = Number.isFinite(Number(width)) && Number(width) > 0 ? Math.round(Number(width)) : 0;
    const nextHeight = Number.isFinite(Number(height)) && Number(height) > 0 ? Math.round(Number(height)) : 0;
  ...
  const hasFixedViewport = computed(() => runtimeViewport.value.width > 0 && runtimeViewport.value.height > 0);
  ...
  const bannerStyle = computed<CSSProperties>(() => {
    ...
    const fallbackBackground = normalizedPageSettings.value.background.scope === "banner" ? normalizedPageSettings.value.background : null;
