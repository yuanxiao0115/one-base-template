<script setup lang="ts">
  ...
  function onMessage(e: MessageEvent) {
    ...
    loadTabLayout(nextTabId).catch((error) => {
      console.warn("[PortalPreviewRenderPage] 收到刷新消息后重载失败", error);
    });
  }

  onMounted(() => {
    window.addEventListener("message", onMessage);
    loadTabLayout(tabId.value).catch((error) => {
      console.warn("[PortalPreviewRenderPage] 初次加载失败", error);
    });
  });
  ...
  <div v-loading="loading" class="page">
    <div v-if="errorMessage" class="error">
      <el-result icon="error" title="加载失败" :sub-title="errorMessage">
        <template #extra> <el-button @click="loadTabLayout(tabId)">重试</el-button> </template>
      </el-result>
    </div>

    <div v-else class="content"><PortalGridRenderer :layout-items :materials-map :page-setting-data /></div>
  </div>
</template>
