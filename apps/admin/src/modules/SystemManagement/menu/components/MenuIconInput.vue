...
type IconfontJsonGlyph = {
  name?: string
  font_class?: string
}

type IconfontJsonResponse = {
  glyphs?: IconfontJsonGlyph[]
}

type MenuIconCandidate = {
  key: string
  name: string
  icon: string
  searchText: string
...
const TAB_ABBR_HINT = 'CP=产品 Iconfont · DJ=党建 Iconfont · OM=OM Iconfont · OD=公文 Iconfont · EP=Element Plus · RI=Remix Icon';
...
async function ensureIconfontLoaded () {
  const hasLoadedAll = MENU_ICONFONT_SOURCES.every((source) => iconfontCandidateCache.has(source.key));
  if (hasLoadedAll) {
  ...
  iconfontLoadTask = Promise.all(MENU_ICONFONT_SOURCES.map(async (source) => {
