...
interface PermissionTreeRawNode {
  id?: number | string | null;
...
export const roleApi = {
  ...
  add: async (data: RoleSavePayload) =>
    getHttpClient().post<BizResponse<boolean>>("/cmict/admin/role/add", {
      data,
    }),

  update: async (data: RoleSavePayload) =>
    getHttpClient().post<BizResponse<boolean>>("/cmict/admin/role/update", {
      data,
    }),

  remove: async (data: { idList: string[] }) =>
    getHttpClient().post<BizResponse<boolean>>("/cmict/admin/role/delete", {
      data,
    }),
