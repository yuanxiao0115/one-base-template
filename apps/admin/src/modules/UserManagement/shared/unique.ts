type BizBooleanResponse = {
  code: number
  data?: boolean | null
  message?: string
}

export type UserUniqueSnapshot = {
  userAccount: string
  phone: string
  mail: string
}

export type OrgUniqueSnapshot = {
  orgName: string
  parentId: string
}

export type PositionUniqueSnapshot = {
  postName: string
}
...
export function shouldCheckUserUnique (current: UserUniqueSnapshot,
  baseline?: UserUniqueSnapshot | null): boolean {
  if (!baseline) {
    return true;
  }
...
export function shouldCheckOrgUnique (current: OrgUniqueSnapshot,
  baseline?: OrgUniqueSnapshot | null): boolean {
  if (!baseline) {
    return true;
  }
...
export function toPositionUniqueSnapshot (input: {
  postName?: unknown
}): PositionUniqueSnapshot {
  return {
    postName: normalizeText(input.postName)
  };
}

export function shouldCheckPositionUnique (current: PositionUniqueSnapshot,
  baseline?: PositionUniqueSnapshot | null): boolean {
  if (!baseline) {
    return true;
  }
