...
import { extractList, toBooleanValue, toNumberValue, toRecord, toStringValue } from "@/shared/api/normalize";
...
interface UserRawRole {
  id?: number | string | null;
  roleName?: string | null;
}

interface UserRawCorporateUser {
  id?: number | string | null;
...
interface PositionRawItem {
  id?: number | string | null;
  postName?: string | null;
}

interface RoleRawItem {
  id?: number | string | null;
  roleName?: string | null;
}

interface UserDetailRawData {
...
function toUserOrg(item: UserRawOrg): UserOrgRecord {
  ...
  return {
    ...
    orgRankType: item.orgRankType == null || item.orgRankType === "" ? null : toNumberValue(item.orgRankType),
...
export const userApi = {
  ...
  add: async (data: UserSavePayload) =>
    getHttpClient().post<BizResponse<boolean>>("/cmict/admin/user/add", {
      data,
    }),

  update: async (data: UserSavePayload) =>
    getHttpClient().post<BizResponse<boolean>>("/cmict/admin/user/update", {
      data,
    }),

  remove: async (data: { id: string }) =>
    getHttpClient().post<BizResponse<boolean>>("/cmict/admin/user/delete", {
      data,
    }),
  ...
  updateStatus: async (data: UserStatusPayload) =>
    getHttpClient().post<BizResponse<boolean>>("/cmict/admin/user/state", {
      data,
    }),
