...
<template>
  <el-form ref="formRef" :model :rules="formRules" label-position="top" :disabled="props.disabled">
    <el-form-item label="当前用户名"> <span>{{ model.nickName }}（{{ model.phone }}）</span> </el-form-item>

    <el-form-item label="当前登录账号"> <span>{{ model.userAccount }}</span> </el-form-item>
    ...
    <el-form-item v-if="model.isReset === 1" key="el-form-item-2" label="确认密码" prop="newPasswordRepeat">
      <el-input v-model.trim="model.newPasswordRepeat" type="password" show-password placeholder="请输入确认密码" />
    </el-form-item>
  </el-form>
</template>
