<template>
  <div class="carousel-config">
    ...
    <el-form-item label="自动播放">
      <el-switch v-model="modelValue.autoplay" :active-value="true" :inactive-value="false" />
    </el-form-item>

    <el-form-item v-if="modelValue.autoplay" label="播放间隔(毫秒)">
      <el-input-number v-model="modelValue.interval" :min="1000" :max="10000" :step="500" controls-position="right" />
    </el-form-item>

    <el-form-item label="指示器">
      <el-switch v-model="modelValue.indicator" :active-value="true" :inactive-value="false" />
    </el-form-item>
    ...
    <el-form-item label="最多显示项数">
      <el-input-number v-model="modelValue.maxItems" :min="1" :max="20" controls-position="right" />
    </el-form-item>

    <!-- 列表配置部分 -->
    <el-divider content-position="left">列表配置</el-divider>
    <el-form-item label="最多显示条数">
      <el-input-number v-model="modelValue.listMaxItems" :min="1" :max="20" controls-position="right" />
    </el-form-item>
  </div>
</template>
...
<script setup lang="ts">
  ...
  const loadDataByCategory = async (categoryId: string) => {
    ...
    // 获取轮播图数据（仅用于预览）
    try {
      const carouselsRes: any = await cmsApi.getUserCarouselsByCategory(categoryId);
