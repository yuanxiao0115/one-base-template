<template>
  <LayoutDisplay layout="no-image" :schema="props.schema" hide-title>
    <template #content>
      <div class="image-text-column" :class="{ 'image-right': imagePosition === 'right' }">
        <div class="column-image" :style="imageBackgroundStyleObj">
          <div v-if="!imageUrl" class="image-placeholder">暂无图片</div>
          <button v-if="showMore" class="image-more" type="button" :style="moreStyleObj" @click.stop="handleMoreClick">
            {{ moreText }}
          </button>
        </div>
        ...
        <ListStyleDisplay class="column-list" :styles="listStyle">
          ...
          >
            ...
            <ListDate v-if="item.publishTime" class="item-date" :text="formatDate(item.publishTime)" inline />
          </ListItem>
        </ListStyleDisplay>
      </div>
    </template>
...
<script setup lang="ts">
  ...
  const items = computed<ColumnItem[]>(() => {
    const allItems = Array.isArray(dataSource.value.items) ? dataSource.value.items : [];
