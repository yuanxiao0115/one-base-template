<template>
  <LayoutDisplay :layout :schema="props.schema" use-default-title>
    <!-- 内容区域 -->
    <template #content>
      <ListStyleDisplay :styles>
        <ListContainer :column-count :list-type>
...
const defaultImageUrl = computed(() => props.schema?.content?.dataSource?.defaultImageUrl
...
const maxDisplayCount = computed(() => props.schema?.content?.dataSource?.maxDisplayCount);
...
const listType = computed(() => props.schema?.content?.dataSource?.listType);

const columnCount = computed(() => props.schema?.content?.dataSource?.columnCount);

const showDot = computed(() => props.schema?.content?.dataSource?.showDot);
...
const styles = computed<ListStyleModelType & StylesObject>(() => props.schema?.style?.list || {});
