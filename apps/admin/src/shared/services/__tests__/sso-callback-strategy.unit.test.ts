...
describe("shared/services/sso-callback-strategy", () => {
  ...
  it("无有效参数时应抛错", async () => {
    ...
    await expect(executeSsoCallbackStrategy({ searchParams: new URLSearchParams(), handlers })).rejects.toThrowError(
      "登录参数无效"
    );
  });
  ...
  it("命中分支时 handler 抛错应透传异常", async () => {
    ...
    await expect(executeSsoCallbackStrategy({ searchParams: sp, handlers })).rejects.toBe(expectedError);
  });
});
