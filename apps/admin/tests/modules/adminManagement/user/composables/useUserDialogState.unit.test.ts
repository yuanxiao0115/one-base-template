...
vi.mock('@/services/security/crypto', () => cryptoMocks);
...
vi.mock('@/modules/adminManagement/user/api', () => ({
  userApi: apiMocks
}));

import { useUserDialogState } from '@/modules/adminManagement/user/composables/useUserDialogState';
...
describe('UserManagement/user/useUserDialogState', () => {
  ...
  it('关联账号搜索错误提示应由弹窗状态统一处理', () => {
    const { dialogState, unmount } = mountUseUserDialogState();

    dialogState.actions.handleBindSearchError(new Error('搜索关联账号失败'));
