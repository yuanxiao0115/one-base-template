...
export const cmsApi = {
  ...
  getUserArticlesByCategory: async (
    ...
    obHttp().get<BizResponse<unknown>>(
      `${portalEndpoints.cms.userArticleListPrefix}/${category}/article/list`,
      {
        params: {
  ...
  getUserCarouselsByCategory: async (category: string) =>
    obHttp().get<BizResponse<unknown>>(
      `${portalEndpoints.cms.userCarouselListPrefix}/${category}/carousel/list`,
      {
        params: {
          pageSize: 20,
          currentPage: 1
