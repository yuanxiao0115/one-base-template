<script setup lang="ts">
  ...
  async function getTargetPath() {
    const res = (await getPortalFrontConfig()) as BizResponse<{ enable?: boolean; customUrl?: string }>;
  ...
  async function doBasicLogin(payload: { username: string; password: string }) {
    ...
    try {
      ...
      await router.replace(resolvePortalLoginTarget({ redirect: route.query.redirect, fallback: "/portal/index" }));
    } catch (error) {
...
<template>
  <div class="portal-login" :style="{ backgroundImage: backgroundImage ? `url(${backgroundImage})` : undefined }">
