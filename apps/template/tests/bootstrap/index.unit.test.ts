...
const mocks = vi.hoisted(() => {
  ...
  const router = {
    beforeEach: vi.fn(),
    currentRoute: { value: { path: '/home/index', meta: {} } },
    replace: vi.fn()
  };
...
import { bootstrapTemplateApp } from '@/bootstrap/index';
...
describe('bootstrap/index', () => {
  ...
  it('应在 bootstrap 阶段注册消息工具并安装壳层插件', async () => {
    await bootstrapTemplateApp();

    expect(mocks.registerMessageUtils).toHaveBeenCalledWith(mocks.app);
    expect(mocks.installAppShellPlugins).toHaveBeenCalledTimes(1);
  });
  ...
  it('应为已登录访问 /login 注入可扩展的回跳解析器', async () => {
    ...
    const resolved = guardOptions?.resolveAuthedLoginRedirect?.({
      to: {
        query: {
          redirect: '/template/demo/about'
        }
      }
    });

    expect(mocks.resolveAppRedirectTarget).toHaveBeenCalledWith('/template/demo/about', {
      fallback: '/',
      baseUrl: '/'
    });
