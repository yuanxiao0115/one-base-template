...
import EditInfoForm from './edit-info-form-jc.vue';
...
const importTemplate = async () => {
  try {
    ...
    const response = await fetch(`/pettion-jc.xlsx?t=${timestamp}`);
    if (!response.ok) throw new Error('文件加载失败');
    const blob = await response.blob();
    const url = URL.createObjectURL(blob);
    const anchor = document.createElement('a');
    anchor.href = url;
    anchor.download = '涉法涉诉(检察)导入模板.xlsx';
...
async function importExcel(param) {
  const { code } = await jcApi.import({ file: param.file });
  try {
    if (code === 200) {
...
const getPettionList = async () => {
  try {
    ...
    const { code, data } = await jcApi.list(params);
    if (code === 1) {
...
const drawerOpt = reactive({
  ...
  addApi: jcApi.add,
  updateApi: jcApi.update,
  detailApi: jcApi.detail,
...
<template>
  ...
  <el-dialog v-model="visiable" title="详情" width="800px" append-to-body>
    <detail :detailData="detailData" :type="'JC'" />
  </el-dialog>
