...
describe('router/assemble-routes', () => {
  ...
  it('system-sfss 模块应装配老项目同路径路由', async () => {
    const { routes } = await buildAppRoutes(createRouteAssemblyOptions(['system-sfss']));
    const allRoutes = flattenRoutes(routes);

    const sunshineParentRoute = allRoutes.find(
      (item) =>
        isSamePath(item.path, '/law-supervison/sunshine-petition') &&
        item.name === 'SystemSfssSunshinePetition'
    );
    const lawsuitLeafRoute = allRoutes.find(
      (item) =>
        isSamePath(item.path, '/lawsuit-petitions/litigation-related/police') &&
        item.name === 'SystemSfssLitigationPolice'
    );
    const processingLeafRoute = allRoutes.find(
      (item) =>
        isSamePath(item.path, '/law-supervison/petition-processing/petition-evaluation') &&
        item.name === 'SystemSfssPetitionProcessingEvaluation'
    );

    expect(sunshineParentRoute?.redirect).toBe('/law-supervison/sunshine-petition/shi');
    expect(lawsuitLeafRoute).toBeDefined();
    expect(processingLeafRoute).toBeDefined();
  });
});
