...
- `user` 已补齐本轮 CR 关注的性能与可维护性问题：首屏只阻塞组织树、模板下载尊重 `baseUrl`、关联账号搜索错误提示通过 `onSearchError -> useUserDialogState` 接管且保留旧请求失败不误提示的竞态保护、页面保持场景对象编排但通过 `reactive(...)` 提供模板友好视图。
- 文档与记录已同步到 `apps/admin/AGENTS.md`、`apps/docs/docs/guide/admin-agent-redlines.md`、`docs/plans/2026-03-20-role-assign-page-state-split.md` 以及 `.codex/*.md`。
