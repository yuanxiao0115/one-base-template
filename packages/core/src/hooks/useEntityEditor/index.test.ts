...
import { useEntityEditor } from './index'
...
describe('useEntityEditor', () => {
  ...
  it('confirm(create/edit) 会先校验再提交并触发成功回调', async () => {
    ...
    const buildPayload = vi.fn(async ({ form }: { form: { name: string } }) => ({ payloadName: form.name }))
    const request = vi.fn(async () => ({ id: 'saved-1' }))
  ...
  it('支持通过外部 formRef 引用触发校验', async () => {
    ...
    const request = vi.fn(async () => ({ id: 'saved-2' }))
