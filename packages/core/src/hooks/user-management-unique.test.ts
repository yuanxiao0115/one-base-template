...
describe('user-management unique helpers', () => {
  it('assertUniqueCheck 在 code 非 200 时抛出业务错误', () => {
    expect(() => assertUniqueCheck({ code: 500, data: false, message: '服务异常' }, '校验失败')).toThrow('服务异常')
  })
  ...
  it('shouldCheckUserUnique 在编辑未变化时返回 false', () => {
    const baseline = toUserUniqueSnapshot({ userAccount: 'admin', phone: '13800000000', mail: 'a@b.com' })
    const current = toUserUniqueSnapshot({ userAccount: 'admin', phone: '13800000000', mail: 'a@b.com' })
  ...
  it('shouldCheckUserUnique 在新增或字段变化时返回 true', () => {
    const baseline = toUserUniqueSnapshot({ userAccount: 'admin', phone: '13800000000', mail: 'a@b.com' })
    const current = toUserUniqueSnapshot({ userAccount: 'admin2', phone: '13800000000', mail: 'a@b.com' })
