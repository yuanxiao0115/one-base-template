...
function createUsage() {
  return [
    '用法: pnpm new:module:item <item-id> --module <module-id> [--title 页面标题] [--route 路由路径] [--dry-run]',
    '示例: pnpm new:module:item user --module demo-management --title "用户管理" --route /demo/management/user'
  ].join('\n');
}
...
function createFiles(params) {
  const { moduleId, itemId, title, routePath } = params;
...
export async function scaffoldModuleItem(options) {
  ...
  const moduleDir = path.join(rootDir, 'src/modules', moduleId);
