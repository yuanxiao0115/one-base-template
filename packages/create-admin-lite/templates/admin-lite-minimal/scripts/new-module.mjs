...
function createUsage() {
  return [
    '用法: pnpm new:module <module-id> [--title 模块标题] [--route 路由前缀] [--tier core|optional] [--enabled-by-default] [--dry-run]',
    '示例: pnpm new:module demo-management --title "Demo 管理" --route demo/management'
  ].join('\n');
}
...
<template>
  <el-card>
    ...
    <p class="text-sm text-[var(--el-text-color-regular)]">
      该模块使用 legacy 聚合路由模板，子业务请通过 'pnpm new:module:item' 继续生成。
    </p>
  </el-card>
</template>
`,
...
export async function scaffoldModule(options) {
  ...
  if (moduleTier === 'optional' && enabledByDefault) {
    throw new Error(
      'optional 模块不能设置 --enabled-by-default；如需默认启用，请使用 --tier core。'
    );
  }
  ...
  const appModulesDir = path.join(rootDir, 'src/modules');
  if (!(await pathExists(appModulesDir))) {
    throw new Error(`未找到项目模块目录: ${appModulesDir}`);
  }
