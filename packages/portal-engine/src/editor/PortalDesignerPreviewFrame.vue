...
const activePreviewScale = computed(() =>
  manualMode.value ? manualPreviewScale.value : autoPreviewScale.value
);
const previewScalePercent = computed(() => Math.round(activePreviewScale.value * 100));
const previewWidth = computed(() =>
  Math.max(1, Math.floor(props.viewportWidth * activePreviewScale.value))
);
const previewHeight = computed(() =>
  Math.max(1, Math.floor(props.viewportHeight * activePreviewScale.value))
);
...
function recalcPreviewScale() {
  ...
  const nextScale = calcPreviewScale(
    innerSize.width,
    innerSize.height,
    props.viewportWidth,
...
function updateManualScale(scalePercent: number) {
  const nextScale =
    clampPortalPreviewPercent(scalePercent, MIN_MANUAL_SCALE_PERCENT, MAX_MANUAL_SCALE_PERCENT) /
    100;
  if (!manualMode.value) {
  ...
  <div class="preview-host">
    <div v-if="!props.templateId" class="empty">
      <el-result
        icon="warning"
        title="缺少参数"
        sub-title="请通过 /portal/design?id=xxx 进入工作台"
      />
    </div>
    <div v-else-if="!props.currentTabId" class="empty">
      <el-result icon="info" title="请先选择页面" sub-title="从左侧选择一个页面，或先新建空白页">
        <template #extra>
          <el-button type="primary" size="small" :icon="Plus" @click="emit('create-root')"
            >新建页面</el-button
          >
        </template>
      </el-result>
