<script setup lang="ts">
  ...
  import { type PortalLayoutItem, resolveLayoutItemComponentName, usePortalPageLayoutStore } from '../stores/pageLayout';
  ...
  const tabs = computed(() => normalizeTabContainerTabs<PortalLayoutItem>(props.item?.component?.cmptConfig?.content?.tabs));
  const tabsStyleConfig = computed<Record<string, unknown>>(() => props.item?.component?.cmptConfig?.style?.tabs || {});
  const containerContentConfig = computed(() => props.item?.component?.cmptConfig?.content?.container || {});
  const containerStyleConfig = computed(() => props.item?.component?.cmptConfig?.style?.container || {});

  const currentTabLayoutItems = computed(() => pageLayoutStore.getTabChildLayoutItems(props.item.i, activeTabId.value));
  ...
  function commitTabs(nextTabs: TabContainerTab<PortalLayoutItem>[], nextActiveTabId?: string) {
    pageLayoutStore.updateTabContainerTabs(props.item.i, nextTabs, nextActiveTabId || activeTabId.value);
  }
  ...
  function onDrop(e: DragEvent) {
    ...
    const container = (e.currentTarget instanceof HTMLElement ? e.currentTarget : tabCanvasRef.value) || null;
    ...
    const droppedComponentName = (payload.cmptConfig as Record<string, unknown> | undefined)?.index as
      ...
      | undefined;
    const droppedIndexName = typeof droppedComponentName?.name === 'string' ? droppedComponentName.name : '';
              ...
              >
                ...
                >
                  <div class="tab-container-editor__item-inner">
                    <button class="tab-container-editor__item-delete" type="button" @click.stop="() => handleDeleteChild(child)">
                      删除
                    </button>
                    ...
                    />
                    <div v-else-if="isNestedTabContainer(child)" class="tab-container-editor__nested-blocked">
