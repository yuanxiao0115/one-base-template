<template>
  <UnifiedContainerDisplay :content="containerContentConfig" :style="containerStyleConfig">
    <div class="app-entrance" :style="wrapperStyleObj">
      ...
      >
        <span v-if="item.badge" class="app-entrance__badge" :style="badgeStyleObj">{{ item.badge }}</span>

        <div class="app-entrance__media" :style="mediaStyleObj">
          <img v-if="item.imageSrc" :src="item.imageSrc" :alt="item.title" class="app-entrance__image" />
          <MenuIcon v-else-if="item.icon" :icon="item.icon" class="app-entrance__icon" :style="iconStyleObj" />
        ...
        <p v-if="showDescription" class="app-entrance__description" :style="descriptionStyleObj">{{ item.description }}</p>
      </button>
    </div>
...
<script setup lang="ts">
  ...
  const columnCount = computed(() => Math.min(6, Math.max(1, Number(entranceConfig.value.columnCount) || 4)));
