    ...
    <el-form label-position="top">
      <ObCard title="按钮样式">
        <el-form-item label="按钮间距(px)">
          <el-input-number v-model="sectionData.buttons.gap" :min="0" :max="40" controls-position="right" />
        </el-form-item>
...
<script setup lang="ts">
  ...
  sectionData.buttons = {
    ...
    align:
      sectionData.buttons?.align === 'center' || sectionData.buttons?.align === 'right' ? sectionData.buttons.align : 'left',
    gap: Number.isFinite(Number(sectionData.buttons?.gap)) ? Number(sectionData.buttons.gap) : 10,
    size: sectionData.buttons?.size === 'large' || sectionData.buttons?.size === 'small' ? sectionData.buttons.size : 'default',
