    ...
    <div class="base-carousel" :style="carouselRootStyleObj">
      ...
      >
        <el-carousel-item v-for="item in slides" :key="item.id">
          ...
          >
            <img v-if="item.image" class="carousel-item__image" :src="item.image" :style="imageStyleObj" alt="carousel">
            <div v-else class="carousel-item__placeholder">请在属性面板配置轮播图片</div>
            <div class="carousel-item__mask" :style="maskStyleObj">
              <div class="carousel-item__title" :style="titleStyleObj">{{ item.title || '轮播标题' }}</div>
              <div v-if="item.subtitle" class="carousel-item__subtitle" :style="subtitleStyleObj">{{ item.subtitle }}</div>
            </div>
          </a>
...
<script setup lang="ts">
  ...
  const interval = computed(() => Math.max(1000, toPositiveNumber(contentConfig.value.interval, 4000)));
  const trigger = computed<TriggerType>(() => (contentConfig.value.trigger === 'click' ? 'click' : 'hover'));
