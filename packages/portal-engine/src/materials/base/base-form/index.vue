<template>
  <UnifiedContainerDisplay :content="containerContentConfig" :style="containerStyleConfig">
    <el-form ref="formRef" :model="formModel" label-position="left" :label-width="`${formStyle.labelWidth}px`" class="base-form">
      <el-form-item
      ...
      <div class="base-form__actions" :style="actionStyleObj">
        <el-button :style="submitButtonStyle" :loading="submitting" @click="submitForm">{{ submitText }}</el-button>
...
<script setup lang="ts">
  ...
  import { parseJsonArray, parseJsonObject, resolveValueByPath, toNonNegativeNumber, toPositiveNumber } from '../common/material-utils';
  ...
  const formStyle = computed(() => {
    ...
    return {
      ...
      buttonAlign: style.buttonAlign === 'center' || style.buttonAlign === 'right' ? style.buttonAlign : 'left',
  ...
  const normalizedFields = computed<FormField[]>(() => {
    ...
    return fields
      .map((item, index) => {
        ...
        const type =
          source.type === 'textarea' || source.type === 'number' || source.type === 'select' || source.type === 'date'
            ? source.type
            : 'text';
  ...
  async function submitForm() {
    ...
    try {
      const method: SubmitMethodType = formConfig.value.method === 'PUT' || formConfig.value.method === 'PATCH' ? formConfig.value.method : 'POST';
      ...
      const link = resolvePortalLink(formLinkConfig.value, resolveValueByPath(responsePayload, formLinkConfig.value.valueKey || 'id'));
      openPortalLink({
        link,
