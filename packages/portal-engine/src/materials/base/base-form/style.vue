    ...
    <el-form label-position="top">
      <ObCard title="表单样式">
        <el-form-item label="标签宽度(px)">
          <el-input-number v-model="sectionData.form.labelWidth" :min="60" :max="180" controls-position="right" />
        </el-form-item>

        <el-form-item label="字段间距(px)">
          <el-input-number v-model="sectionData.form.fieldGap" :min="0" :max="40" controls-position="right" />
        </el-form-item>

        <el-form-item label="输入框高度(px)">
          <el-input-number v-model="sectionData.form.inputHeight" :min="30" :max="72" controls-position="right" />
        </el-form-item>
        ...
        <el-form-item label="按钮间距(px)">
          <el-input-number v-model="sectionData.form.buttonGap" :min="0" :max="40" controls-position="right" />
        </el-form-item>

        <el-form-item label="主按钮颜色">
          <PortalColorField v-model="sectionData.form.primaryColor" show-alpha />
        </el-form-item>
      </ObCard>
    </el-form>
  </div>
...
<script setup lang="ts">
  ...
  sectionData.form = {
    ...
    fieldGap: Number.isFinite(Number(sectionData.form?.fieldGap)) ? Number(sectionData.form.fieldGap) : 12,
    inputHeight: Number(sectionData.form?.inputHeight) > 0 ? Number(sectionData.form.inputHeight) : 40,
    buttonAlign:
      sectionData.form?.buttonAlign === 'center' || sectionData.form?.buttonAlign === 'right'
        ? sectionData.form.buttonAlign
        : 'left',
    buttonGap: Number.isFinite(Number(sectionData.form?.buttonGap)) ? Number(sectionData.form.buttonGap) : 10,
