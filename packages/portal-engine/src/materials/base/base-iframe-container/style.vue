    ...
    <el-form label-position="top">
      <ObCard title="Iframe样式">
        <el-form-item label="高度(px)">
          <el-input-number
            v-model="sectionData.iframe.height"
            :min="160"
            :max="1800"
        ...
        <el-form-item label="背景色">
          <ObColorField v-model="sectionData.iframe.backgroundColor" show-alpha />
        </el-form-item>
...
sectionData.iframe = {
  ...
  borderWidth: Number.isFinite(Number(sectionData.iframe?.borderWidth))
    ? Number(sectionData.iframe.borderWidth)
    : 1,
  borderColor: sectionData.iframe?.borderColor || '#e2e8f0',
  borderRadius: Number.isFinite(Number(sectionData.iframe?.borderRadius))
    ? Number(sectionData.iframe.borderRadius)
    : 8,
