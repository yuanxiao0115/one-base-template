    ...
    <el-form label-position="top">
      <ObCard title="图片内容">
        ...
        <el-form-item label="替代文本 alt">
          <el-input v-model.trim="sectionData.image.alt" maxlength="80" show-word-limit placeholder="用于无障碍与图片加载失败场景" />
        </el-form-item>
        ...
        <el-form-item label="新窗口打开">
          <el-switch v-model="sectionData.image.openInNewTab" :disabled="!sectionData.image.linkUrl" />
        </el-form-item>
      </ObCard>
      ...
      <ObCard title="上传配置">
        ...
        <template v-if="sectionData.upload.enabled">
          ...
          <el-form-item label="文件字段名">
            <el-input v-model.trim="sectionData.upload.fileField" maxlength="32" show-word-limit placeholder="默认 file" />
          </el-form-item>

          <el-form-item label="上传文件大小限制(MB)">
            <el-input-number v-model="sectionData.upload.maxSizeMb" :min="1" :max="100" controls-position="right" />
          </el-form-item>
          ...
          <el-form-item label="上传图片">
            <div class="upload-actions">
              <el-upload
                ...
                :disabled="uploading"
              >
                <el-button type="primary" plain :icon="UploadFilled" :loading="uploading">上传图片</el-button>
              </el-upload>
              <el-button :disabled="!sectionData.image.src" @click="sectionData.image.src = ''">清空图片值</el-button>
            </div>
...
<script setup lang="ts">
  ...
  import { normalizeImageSource, parseJsonObject, resolveValueByPath, toPositiveNumber } from '../common/material-utils';
  ...
  sectionData.upload = {
    ...
    fileField: typeof sectionData.upload?.fileField === 'string' && sectionData.upload.fileField.trim()
      ? sectionData.upload.fileField
      : 'file',
    maxSizeMb: Math.max(1, toPositiveNumber(sectionData.upload?.maxSizeMb, 5)),
    responseImagePath:
      typeof sectionData.upload?.responseImagePath === 'string' && sectionData.upload.responseImagePath.trim()
        ? sectionData.upload.responseImagePath
        : 'data.id',
    headersJson: typeof sectionData.upload?.headersJson === 'string' ? sectionData.upload.headersJson : '{}',
