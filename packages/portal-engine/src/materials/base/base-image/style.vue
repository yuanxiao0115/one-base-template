    ...
    <el-form label-position="top">
      <ObCard title="图片样式">
        <el-form-item label="高度(px)">
          <el-input-number
            v-model="sectionData.image.height"
            :min="80"
            :max="1200"
        ...
        <el-form-item v-if="sectionData.image.widthMode === 'custom'" label="宽度(px)">
          <el-input-number
            v-model="sectionData.image.width"
            :min="120"
            :max="1600"
            controls-position="right"
          />
        ...
        <el-form-item label="背景色">
          <ObColorField v-model="sectionData.image.backgroundColor" show-alpha />
        </el-form-item>
      ...
      <ObCard title="图片说明样式">
        <el-form-item label="说明文字颜色">
          <ObColorField v-model="sectionData.image.captionColor" show-alpha />
        </el-form-item>

        <el-form-item label="说明字号(px)">
          <el-input-number
            v-model="sectionData.image.captionFontSize"
            :min="12"
            :max="40"
            controls-position="right"
          />
        ...
        <el-form-item label="说明上间距(px)">
          <el-input-number
            v-model="sectionData.image.captionMarginTop"
            :min="0"
            :max="100"
            controls-position="right"
          />
...
sectionData.image = {
  ...
  borderWidth: Number.isFinite(Number(sectionData.image?.borderWidth))
    ? Number(sectionData.image.borderWidth)
    : 0,
  borderColor: sectionData.image?.borderColor || '#e2e8f0',
  borderRadius: Number.isFinite(Number(sectionData.image?.borderRadius))
    ? Number(sectionData.image.borderRadius)
    : 8,
  boxShadow: sectionData.image?.boxShadow || 'none',
  captionColor: sectionData.image?.captionColor || '#475569',
  captionFontSize:
    Number(sectionData.image?.captionFontSize) > 0 ? Number(sectionData.image.captionFontSize) : 13,
