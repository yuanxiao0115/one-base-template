    ...
    <el-form label-position="top">
      ...
      <ObCard title="字段映射">
        <el-form-item label="标题字段 key">
          <el-input v-model.trim="sectionData.stat.titleKey" maxlength="40" show-word-limit placeholder="例如：title" />
        </el-form-item>

        <el-form-item label="数值字段 key">
          <el-input v-model.trim="sectionData.stat.valueKey" maxlength="40" show-word-limit placeholder="例如：value" />
        </el-form-item>

        <el-form-item label="单位字段 key">
          <el-input v-model.trim="sectionData.stat.unitKey" maxlength="40" show-word-limit placeholder="例如：unit" />
        </el-form-item>

        <el-form-item label="趋势字段 key">
          <el-input v-model.trim="sectionData.stat.trendKey" maxlength="40" show-word-limit placeholder="例如：trend" />
        </el-form-item>

        <el-form-item label="主键字段 key">
          <el-input v-model.trim="sectionData.stat.idKey" maxlength="40" show-word-limit placeholder="例如：id" />
        </el-form-item>

        <el-form-item label="最大显示数">
          <el-input-number v-model="sectionData.stat.maxDisplayCount" :min="1" :max="24" controls-position="right" />
        </el-form-item>
      </ObCard>
...
<script setup lang="ts">
  ...
  sectionData.stat = {
    titleKey: typeof sectionData.stat?.titleKey === 'string' && sectionData.stat.titleKey.trim() ? sectionData.stat.titleKey : 'title',
    valueKey: typeof sectionData.stat?.valueKey === 'string' && sectionData.stat.valueKey.trim() ? sectionData.stat.valueKey : 'value',
    unitKey: typeof sectionData.stat?.unitKey === 'string' && sectionData.stat.unitKey.trim() ? sectionData.stat.unitKey : 'unit',
    trendKey: typeof sectionData.stat?.trendKey === 'string' && sectionData.stat.trendKey.trim() ? sectionData.stat.trendKey : 'trend',
    idKey: typeof sectionData.stat?.idKey === 'string' && sectionData.stat.idKey.trim() ? sectionData.stat.idKey : 'id',
    maxDisplayCount: Number(sectionData.stat?.maxDisplayCount) > 0 ? Number(sectionData.stat.maxDisplayCount) : 8,
