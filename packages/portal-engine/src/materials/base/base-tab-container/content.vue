      ...
      <ObCard title="页签配置">
        ...
        <div v-if="sectionData.tabs.length > 0" class="tabs-list">
          <div v-for="tab in sectionData.tabs" :key="tab.id" class="tabs-item">
            ...
            <el-input v-model.trim="tab.title" maxlength="24" show-word-limit placeholder="页签标题" />

            <div class="tabs-item__meta">ID: {{ tab.id }} · 子组件: {{ tab.layoutItems.length }}</div>
...
<script setup lang="ts">
  ...
  import { createTabContainerTabId, normalizeTabContainerTabs, resolveTabContainerActiveTabId } from '../../../schema/tab-container';
  ...
  function syncFromSchema() {
    ...
    sectionData.activeTabId = resolveTabContainerActiveTabId(sectionData.tabs, props.schema?.activeTabId);
  ...
  function removeTab(tabId: string) {
    ...
    sectionData.activeTabId = resolveTabContainerActiveTabId(sectionData.tabs, sectionData.activeTabId);
  }
