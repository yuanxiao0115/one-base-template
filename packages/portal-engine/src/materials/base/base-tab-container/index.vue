<template>
  <UnifiedContainerDisplay :content="containerContentConfig" :style="containerStyleConfig">
    <div class="base-tab-container" :style="tabsVars">
      <el-tabs
        v-model="activeTabId"
        class="base-tab-container__tabs"
...
const tabs = computed(() =>
  normalizeTabContainerTabs<PortalLayoutItem>(props.schema?.content?.tabs)
);
...
watch(
  ...
  () => {
    const resolvedSchemaActiveTabId = resolveTabContainerActiveTabId(
      tabs.value,
      props.schema?.content?.activeTabId
    );
