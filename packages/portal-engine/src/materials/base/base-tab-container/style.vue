    ...
    <el-form label-position="top">
      <ObCard title="页签样式">
        ...
        <el-form-item label="未激活字体颜色">
          <PortalColorField v-model="sectionData.tabs.textColor" show-alpha />
        </el-form-item>

        <el-form-item label="激活字体颜色">
          <PortalColorField v-model="sectionData.tabs.activeTextColor" show-alpha />
        </el-form-item>

        <el-form-item label="页签字号(px)">
          <el-input-number v-model="sectionData.tabs.fontSize" :min="12" :max="32" controls-position="right" />
        </el-form-item>

        <el-form-item label="导航下间距(px)">
          <el-input-number v-model="sectionData.tabs.navMarginBottom" :min="0" :max="60" controls-position="right" />
        </el-form-item>

        <el-form-item label="内容区背景">
          <PortalColorField v-model="sectionData.tabs.paneBackgroundColor" show-alpha />
        </el-form-item>
      </ObCard>
...
<script setup lang="ts">
  ...
  sectionData.tabs = {
    ...
    navMarginBottom: Number.isFinite(Number(sectionData.tabs?.navMarginBottom)) ? Number(sectionData.tabs.navMarginBottom) : 12,
    paneBackgroundColor: sectionData.tabs?.paneBackgroundColor || 'transparent',
    panePaddingTop: Number.isFinite(Number(sectionData.tabs?.panePaddingTop)) ? Number(sectionData.tabs.panePaddingTop) : 0,
    panePaddingRight: Number.isFinite(Number(sectionData.tabs?.panePaddingRight)) ? Number(sectionData.tabs.panePaddingRight) : 0,
    panePaddingBottom: Number.isFinite(Number(sectionData.tabs?.panePaddingBottom)) ? Number(sectionData.tabs.panePaddingBottom) : 0,
    panePaddingLeft: Number.isFinite(Number(sectionData.tabs?.panePaddingLeft)) ? Number(sectionData.tabs.panePaddingLeft) : 0,
  };
