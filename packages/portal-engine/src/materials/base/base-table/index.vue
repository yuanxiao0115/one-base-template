...
<script setup lang="ts">
  ...
  const pageSizes = computed(() => {
    const values = Array.isArray(tableConfig.value.pageSizes)
      ? tableConfig.value.pageSizes.map((item) => Math.floor(Number(item))).filter((item) => Number.isFinite(item) && item > 0)
      : [];
  ...
  const resolvedTableProps = computed<Record<string, unknown>>(() => parseJsonObject(tableConfig.value.tablePropsJson));
  ...
  async function loadApiRows() {
    ...
    try {
      ...
      const pageSizeParamKey = String(dataSourceConfig.value.pageSizeParamKey || '').trim() || 'pageSize';
      ...
      const totalValue = totalPath ? resolveValueByPath(responsePayload, totalPath) : apiRows.value.length;
