    ...
    <el-form label-position="top">
      <ObCard title="文本样式">
        ...
        <el-form-item label="文字颜色">
          <PortalColorField v-model="sectionData.text.textColor" show-alpha />
        </el-form-item>

        <el-form-item label="背景色">
          <PortalColorField v-model="sectionData.text.backgroundColor" show-alpha />
        </el-form-item>

        <el-form-item label="字号(px)">
          <el-input-number v-model="sectionData.text.fontSize" :min="12" :max="64" controls-position="right" />
        </el-form-item>
        ...
        <el-form-item label="行高">
          <el-input-number v-model="sectionData.text.lineHeight" :min="1" :max="3" :step="0.1" :precision="1" />
        </el-form-item>

        <el-form-item label="字间距(px)">
          <el-input-number v-model="sectionData.text.letterSpacing" :min="0" :max="30" controls-position="right" />
        </el-form-item>

        <el-form-item label="内边距(px)">
          <el-input-number v-model="sectionData.text.padding" :min="0" :max="120" controls-position="right" />
        </el-form-item>
        ...
        <el-form-item label="最大行数(0为不限制)">
          <el-input-number v-model="sectionData.text.maxLines" :min="0" :max="20" controls-position="right" />
        </el-form-item>
      </ObCard>
    </el-form>
  </div>
...
<script setup lang="ts">
  ...
  sectionData.text = {
    align:
      sectionData.text?.align === 'center' || sectionData.text?.align === 'right' || sectionData.text?.align === 'justify'
        ? sectionData.text.align
        : 'left',
    fontSize: Number(sectionData.text?.fontSize) > 0 ? Number(sectionData.text.fontSize) : 15,
    fontWeight: Number.isFinite(Number(sectionData.text?.fontWeight)) ? Number(sectionData.text.fontWeight) : 400,
    lineHeight: Number(sectionData.text?.lineHeight) > 0 ? Number(sectionData.text.lineHeight) : 1.7,
    letterSpacing: Number.isFinite(Number(sectionData.text?.letterSpacing)) ? Number(sectionData.text.letterSpacing) : 0,
    textColor: sectionData.text?.textColor || '#334155',
    backgroundColor: sectionData.text?.backgroundColor || 'transparent',
    padding: Number.isFinite(Number(sectionData.text?.padding)) ? Number(sectionData.text.padding) : 0,
    borderStyle: sectionData.text?.borderStyle || 'none',
    borderWidth: Number.isFinite(Number(sectionData.text?.borderWidth)) ? Number(sectionData.text.borderWidth) : 0,
    borderColor: sectionData.text?.borderColor || '#e2e8f0',
    borderRadius: Number.isFinite(Number(sectionData.text?.borderRadius)) ? Number(sectionData.text.borderRadius) : 0,
    maxLines: Number.isFinite(Number(sectionData.text?.maxLines)) ? Number(sectionData.text.maxLines) : 0,
  };
