    ...
    <el-form label-position="top">
      ...
      <ObCard title="字段映射">
        <el-form-item label="时间字段 key">
          <el-input v-model.trim="sectionData.timeline.timeKey" maxlength="40" show-word-limit placeholder="例如：publishTime" />
        </el-form-item>

        <el-form-item label="标题字段 key">
          <el-input v-model.trim="sectionData.timeline.titleKey" maxlength="40" show-word-limit placeholder="例如：title" />
        </el-form-item>

        <el-form-item label="内容字段 key">
          <el-input v-model.trim="sectionData.timeline.contentKey" maxlength="40" show-word-limit placeholder="例如：content" />
        </el-form-item>

        <el-form-item label="主键字段 key">
          <el-input v-model.trim="sectionData.timeline.idKey" maxlength="40" show-word-limit placeholder="例如：id" />
        </el-form-item>

        <el-form-item label="最大显示数">
          <el-input-number v-model="sectionData.timeline.maxDisplayCount" :min="1" :max="40" controls-position="right" />
        </el-form-item>
      </ObCard>
...
<script setup lang="ts">
  ...
  if (!sectionData.dataSource.staticRowsJson.trim()) {
    sectionData.dataSource = {
      ...createDefaultPortalDataSourceModel(),
      staticRowsJson: '[{"id":"1","time":"2026-03-10","title":"发布通知","content":"门户基础物料版本发布"}]',
    };
  }
  ...
  sectionData.timeline = {
    ...
    contentKey:
      typeof sectionData.timeline?.contentKey === 'string' && sectionData.timeline.contentKey.trim()
        ...
        : 'content',
    idKey: typeof sectionData.timeline?.idKey === 'string' && sectionData.timeline.idKey.trim() ? sectionData.timeline.idKey : 'id',
    maxDisplayCount: Number(sectionData.timeline?.maxDisplayCount) > 0 ? Number(sectionData.timeline.maxDisplayCount) : 10,
