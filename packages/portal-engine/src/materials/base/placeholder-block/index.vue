...
<script setup lang="ts">
  ...
  const BORDER_STYLE_SET = new Set<PlaceholderBlockBorderStyle>(['none', 'solid', 'dashed', 'dotted']);
  ...
  function toBorderStyle(value: unknown, fallback: PlaceholderBlockBorderStyle): PlaceholderBlockBorderStyle {
  ...
  const blockStyleObj = computed<CSSProperties>(() => {
    ...
    return {
      ...
      border:
        borderWidth > 0 && borderStyle !== 'none' ? `${borderWidth}px ${borderStyle} ${borderColor}` : 'none',
