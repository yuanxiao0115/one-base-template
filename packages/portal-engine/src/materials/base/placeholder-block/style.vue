    ...
    <el-form label-position="top">
      <ObCard title="HTML 区域样式">
        ...
        <el-form-item label="背景色">
          <ObColorField v-model="sectionData.block.backgroundColor" show-alpha />
        </el-form-item>

        <el-form-item label="文字颜色">
          <ObColorField v-model="sectionData.block.textColor" show-alpha />
        </el-form-item>
        ...
        <el-form-item label="行高">
          <el-input-number
            v-model="sectionData.block.lineHeight"
            :min="1"
            :max="3"
            :step="0.1"
            :precision="1"
...
sectionData.block = {
  minHeight: Number.isFinite(Number(sectionData.block?.minHeight))
    ? Number(sectionData.block.minHeight)
    : 180,
  padding: Number.isFinite(Number(sectionData.block?.padding))
    ? Number(sectionData.block.padding)
    : 0,
  backgroundColor: sectionData.block?.backgroundColor || 'transparent',
  textColor: sectionData.block?.textColor || '#0f172a',
  borderWidth: Number.isFinite(Number(sectionData.block?.borderWidth))
    ? Number(sectionData.block.borderWidth)
    : 0,
  borderStyle: sectionData.block?.borderStyle || 'solid',
  borderColor: sectionData.block?.borderColor || 'transparent',
  borderRadius: Number.isFinite(Number(sectionData.block?.borderRadius))
    ? Number(sectionData.block.borderRadius)
    : 0,
  fontSize: Number.isFinite(Number(sectionData.block?.fontSize))
    ? Number(sectionData.block.fontSize)
    : 14,
  lineHeight: Number.isFinite(Number(sectionData.block?.lineHeight))
    ? Number(sectionData.block.lineHeight)
    : 1.6
};
