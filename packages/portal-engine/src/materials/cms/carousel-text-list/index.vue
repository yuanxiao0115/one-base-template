<template>
  <LayoutDisplay layout="carousel-left" :schema="props.schema" use-default-title>
    ...
    <template #carousel>
      ...
      >
        <el-carousel-item v-for="item in carouselItems" :key="item.id">
          <div class="carousel-item">
            ...
            <div v-if="carouselStyle.showTitle && item.carouselTitle" class="carousel-title" :style="getTitleStyle">
              <span class="title-text">{{ item.carouselTitle }}</span>
              <span class="page-indicator" :style="{ color: carouselStyle.pageIndicatorColor }">
                {{ activeIndex + 1 }}
              </span>
      ...
      <div v-else class="empty-carousel"><el-empty :image-size="80" description="暂无轮播图数据" /></div>
    </template>
      ...
      <template v-if="listItems.length > 0">
        ...
        >
          <div class="item-content" @click="() => handleItemClick(item)">
            <div class="item-dot" />
            <div class="item-title" :style="getTitleTextStyle" :title="item.articleTitle">{{ item.articleTitle }}</div>
            <div class="item-date" :style="getDateStyle">{{ item.publishTime.split(" ")[0] }}</div>
          </div>
        </div>
      </template>
...
<script setup lang="ts">
  ...
  const loadDataByCategory = async (categoryId: string) => {
    ...
    try {
      ...
      // 处理轮播图数据
      if ((carouselsRes as any).code === 200 && (carouselsRes as any).data?.records) {
        const maxItems = dataSource.value.maxItems || 5;
        dynamicCarouselItems.value = (carouselsRes as any).data.records.slice(0, maxItems);
      } else {
        dynamicCarouselItems.value = [];
      }

      // 处理列表数据
      if ((articlesRes as any).code === 200 && (articlesRes as any).data?.records) {
        const maxItems = dataSource.value.listMaxItems || 10;
        dynamicListItems.value = (articlesRes as any).data.records.slice(0, maxItems);
      } else {
        dynamicListItems.value = [];
      }
    } catch (error) {
