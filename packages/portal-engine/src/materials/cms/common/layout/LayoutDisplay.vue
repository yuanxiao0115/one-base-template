<template>
  ...
  <div class="cms-layout-container" :style="containerStyleObj">
    ...
    <template v-if="layout === 'no-image'">
      <!-- 标题区域 -->
      <div v-if="isSimpleTitle && !hideTitle" class="layout-title-area"><TitleDisplay :schema /></div>

      <!-- 内容区域 -->
      <div class="layout-content-area" :style="contentContainerStyleObj">
        <slot name="content"> <div class="placeholder-text">内容区域</div> </slot>
      </div>
    </template>
    ...
    <template v-else-if="layout === 'image-top'">
      ...
      <!-- 标题区域 -->
      <div v-if="isSimpleTitle && !hideTitle" class="layout-title-area"><TitleDisplay :schema /></div>
      <div v-else-if="!hideTitle" class="layout-title-area"><slot name="title" /></div>
    ...
    <template v-else-if="layout === 'carousel-left'">
      <div class="layout-horizontal-container">
        <!-- 轮播图区域 -->
        <div class="layout-carousel-area" :style="carouselStyle"><slot name="carousel" /></div>
...
<script setup lang="ts">
    ...
    return {
      ...
      padding:
        style.containerPaddingTop !== undefined
          ...
          : undefined,
      height: showTitle ? (isSimpleTitle.value ? '100%' : 'calc(100% - 62px)') : '100%',
    };
  });
  ...
  const imageContainerStyleObj = computed(() => {
    ...
    return {
      ...
      height: props.layout !== 'no-image' ? addPxUnit(content.imageHeight) : undefined,
    };
  });
...
<style scoped>
  ...
  .layout-content-area {
    ...
    scrollbar-color: rgb(144 147 153 / 0.3) transparent;
  }
