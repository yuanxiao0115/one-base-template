<template>
  <div class="pb-base-title" :style="containerStyleObj">
    ...
    <!-- 蓝色简约样式：左对齐标题 + 条纹线 -->
    <div v-else-if="titleStyle === 'blue-simple'" class="title-blue-wrapper title-wrapper">
      <div class="blue-title-row">
        <div class="blue-title title" :style="blueTitleStyleObj">{{ title }}</div>
    ...
    <!-- 装饰样式：两侧渐变线条和菱形 -->
    <div v-else class="title-decorated-wrapper title-wrapper" :style="titleWrapperStyleObj">
      <div class="left line-container">
        <div class="left line-gradient" :style="leftLineGradientStyle" />
        <div class="diamond-container left">
          <div class="diamond diamond-small left" :style="diamondStyleSmallLeft" />
          <div class="diamond diamond-large left" :style="diamondStyleLargeLeft" />
        </div>
      </div>
      <div class="title" :style="titleStyleObj">{{ title }}</div>
      <div class="line-container right">
        <div class="diamond-container right">
          <div class="diamond diamond-large right" :style="diamondStyleLargeRight" />
          <div class="diamond diamond-small right" :style="diamondStyleSmallRight" />
        </div>
...
<script setup lang="ts">
  ...
  const stripeAccentColor = computed(
    () => styleData.value?.stripeAccentColor || styleData.value?.borderColor || styleData.value?.titleColor || '#2B6DE5'
  );
  ...
  const titleWrapperStyleObj = computed(() => ({
    ...
    width: '100%',
    overflow: 'visible',
    borderBottom: titleStyle.value === 'simple' ? `1px solid ${borderColor.value || '#eaeaea'}` : undefined,
  }));
