<template>
  <div class="links-config">
    <div v-for="(link, index) in modelValue.links" :key="link.id || index" class="link-config">
      <el-card class="link-item-card">
        <div class="link-header">
          <span>链接 #{{ index + 1 }}</span>
          <el-button type="danger" size="small" :icon="Delete" circle @click="() => removeLink(index)" />
        </div>
        <el-form-item label="标题"> <el-input v-model="link.title" placeholder="请输入链接标题" /> </el-form-item>
        <el-form-item label="URL"> <el-input v-model="link.url" placeholder="请输入链接地址" /> </el-form-item>
      </el-card>
    </div>

    <div v-if="(modelValue?.links?.length || 0) === 0" class="empty-links"><el-empty description="暂无链接" /></div>

    <el-button type="primary" :icon="Plus" class="add-link-btn" @click="addLink"> 添加链接 </el-button>
  </div>
</template>
