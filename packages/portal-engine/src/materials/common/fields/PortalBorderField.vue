...
const onWidthChange = (value: number | undefined) =>
  updateValue({ width: toNumber(value, borderValue.value.width) });
const onRadiusChange = (value: number | undefined) =>
  updateValue({ radius: toNumber(value, borderValue.value.radius) });
...
<template>
  ...
  <template v-if="showColorAndWidth">
    <el-form-item :label="colorLabel">
      <ObColorField
        :model-value="borderValue.color"
        :show-alpha="colorShowAlpha"
