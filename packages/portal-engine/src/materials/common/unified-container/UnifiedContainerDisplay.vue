...
const showHeader = computed(
  () =>
    contentConfig.value.showTitle ||
    contentConfig.value.showExternalLink ||
...
const isSubtitleInline = computed(
  () => showSubtitle.value && contentConfig.value.subtitleLayout === 'inline'
);
...
const externalLinkTarget = computed(() =>
  contentConfig.value.openExternalInNewTab ? '_blank' : '_self'
);

const externalLinkRel = computed(() =>
  contentConfig.value.openExternalInNewTab ? 'noopener noreferrer' : undefined
);
      ...
      <div v-if="contentConfig.showTitle" class="unified-container__title-group">
        ...
        <div class="unified-container__title-content">
          <div v-if="isSubtitleInline" class="unified-container__title-line">
            <h3 class="unified-container__title" :style="titleStyleObj">
              {{ contentConfig.title }}
            </h3>
            <p class="unified-container__subtitle" :style="subtitleStyleObj">
