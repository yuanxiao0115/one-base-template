...
export function normalizeTabContainerTabs<TLayoutItem = unknown>(value: unknown): TabContainerTab<TLayoutItem>[] {
  ...
  return value
    .map((raw, index) => {
      ...
      const rawId = item.id;
      const id = typeof rawId === 'string' && rawId.trim().length > 0 ? rawId.trim() : createTabContainerTabId(index + 1);
      const title = typeof item.title === 'string' && item.title.trim().length > 0 ? item.title : `页签${index + 1}`;
      const layoutItems = Array.isArray(item.layoutItems) ? (item.layoutItems as TLayoutItem[]) : [];
