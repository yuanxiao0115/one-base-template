...
interface PortalUploadResponseLike {
  code?: unknown;
  success?: unknown;
  message?: unknown;
...
const pageTabOptions = computed<SelectOption[]>(() => {
  ...
  const walk = (nodes: PortalTab[], parentNames: string[]) => {
    for (const node of nodes) {
      ...
      if (node.tabType === 2) {
        const tabId =
          typeof node.id === 'string' || typeof node.id === 'number' ? String(node.id) : '';
        if (tabId && tabName) {
          options.push({
            value: tabId,
...
function normalizeFormState(source: PortalTemplateDetails): PortalTemplateDetails {
  ...
  const headerEnabled =
    typeof sourceHeaderEnabled === 'boolean'
      ? sourceHeaderEnabled
      : normalized.shell.header.enabled;
  const footerEnabled =
    typeof sourceFooterEnabled === 'boolean'
      ? sourceFooterEnabled
      : normalized.shell.footer.enabled;
...
const schemaDetailsPrettyJson = computed(() =>
  JSON.stringify(buildPortalTemplateDetailsSchemaPreview(), null, 2)
);
    ...
    <section class="drawer-top">
      <div class="top-main">
        ...
        <div class="top-sub-title">
          当前模板页面数：{{ previewTabCount }}。配置编辑时会实时驱动右侧预览，保存后统一写入
          `details`。
        </div>
      </div>

      <div class="top-actions">
        <el-button size="small" :icon="CopyDocument" @click="jsonViewerVisible = true"
          >查看数据结构</el-button
        >
      </div>
    </section>
      ...
      <el-tab-pane name="header" label="页眉组件">
        <div class="tab-content">
          <PortalShellHeaderSettingsForm
            ...
            :page-tab-options="pageTabOptions"
            :upload-resource="props.uploadResource"
            :resolve-resource-url="props.resolveResourceUrl"
            :notify="props.notify"
          />
        </div>
      </el-tab-pane>
      ...
      <section class="json-panel">
        <header class="json-header">
          ...
          <el-button
            ...
            size="small"
            :icon="CopyDocument"
            @click="copyJson(currentDetailsPrettyJson)"
            >复制</el-button
          >
        </header>
      ...
      <section class="json-panel">
        <header class="json-header">
          ...
          <el-button
            ...
            size="small"
            :icon="CopyDocument"
            @click="copyJson(schemaDetailsPrettyJson)"
            >复制</el-button
          >
        </header>
