...
interface PortalTemplateListQuery {
  currentPage: number;
  pageSize: number;
}
...
const props = defineProps<{
  ...
  loadTemplateList?: (query: PortalTemplateListQuery) => Promise<unknown>;
  templateListPageSize?: number;
}>();
...
async function loadPortalTemplateOptions(force = false) {
  ...
  try {
    const res = await props.loadTemplateList({
      currentPage: 1,
      pageSize: Number(props.templateListPageSize) || 200
    });
    const normalizedRes = res as BizResLike | null | undefined;
    if (!normalizeBizOk(normalizedRes)) {
      portalTemplateOptions.value = [];
      portalTemplateOptionsLoaded.value = false;
      return;
    }
    const source =
      normalizedRes && typeof normalizedRes === 'object' && 'data' in normalizedRes
        ? normalizedRes.data
        : normalizedRes;
    portalTemplateOptions.value = normalizePortalTemplateOptions(source);
    portalTemplateOptionsLoaded.value = true;
  } catch {
    portalTemplateOptions.value = [];
    portalTemplateOptionsLoaded.value = false;
...
async function onSubmit() {
  ...
  const normalizedUrl =
    form.tabType === 4 ? buildPortalTemplateLink(form.portalTemplateId) : form.tabUrl.trim();
  ...
  <el-dialog v-model="visible" :title="title" width="640px" :close-on-click-modal="false">
    <el-form ref="formRef" :model="form" :rules label-position="top">
      ...
      <el-form-item label="类型" prop="tabType">
        <el-radio-group v-model="form.tabType" :disabled="dialogMode === 'edit'">
          <el-radio v-for="opt in tabTypeOptions" :key="opt.key" :value="opt.key">{{
            opt.label
          }}</el-radio>
        </el-radio-group>
      </el-form-item>
      ...
      <template v-if="isLinkType">
        <el-form-item v-if="isPortalListType" label="门户列表" prop="portalTemplateId">
          ...
          >
            <el-option
              v-for="item in portalTemplateOptions"
              :key="item.id"
              :label="item.name"
              :value="item.id"
            />
        ...
        <el-form-item label="打开方式" prop="tabUrlOpenMode">
          <el-radio-group v-model="form.tabUrlOpenMode">
            <el-radio v-for="opt in openModeOptions" :key="opt.key" :value="opt.key">{{
              opt.label
            }}</el-radio>
          </el-radio-group>
        </el-form-item>
        ...
        <el-form-item label="单点方式" prop="tabUrlSsoType">
          <el-radio-group v-model="form.tabUrlSsoType">
            <el-radio v-for="opt in ssoTypeOptions" :key="opt.key" :value="opt.key">{{
              opt.label
            }}</el-radio>
          </el-radio-group>
        </el-form-item>
    ...
    <template #footer>
      <div class="footer">
        ...
        <el-button type="primary" :loading="Boolean(props.loading)" @click="onSubmit">
