...
import { isPortalTabEditable } from '../domain/tab-tree';
  ...
  <div class="tree-wrap">
    ...
    >
      <template #default="{ data }">
        <div
          class="node"
          :class="{ 'node--active': isCurrentNode(data), 'node--hidden': data.isHide === 1 }"
        >
          <div class="main">
            <el-icon class="node-type-icon">
              <component :is="getTypeIcon(data.tabType)" />
            </el-icon>
            <span class="name truncate" :title="data.tabName || '未命名'">{{
              data.tabName || '未命名'
            }}</span>
            ...
            <el-dropdown trigger="click" @command="(cmd: string) => onCommand(cmd, data)">
              ...
              <template #dropdown>
                <el-dropdown-menu>
                  ...
                  <el-dropdown-item v-if="data.tabType === 1" command="child"
                    >新建子级</el-dropdown-item
                  >
                </el-dropdown-menu>
              </template>
            </el-dropdown>
