...
interface PortalShellSettingsNotify {
  success?: (text: string) => void;
  error?: (text: string) => void;
}

interface PortalUploadDataLike {
  id?: unknown;
  joinUrl?: unknown;
  savedPath?: unknown;
}

interface PortalUploadResponseLike extends BizResLike {
  data?: PortalUploadDataLike | unknown;
}

const props = defineProps<{
  formState: PortalTemplateDetails;
  pageTabOptions: SelectOption[];
  uploadResource?: ((file: File) => Promise<PortalUploadResponseLike>) | null;
  resolveResourceUrl?: ((resourceId: string) => string) | null;
  notify?: PortalShellSettingsNotify;
}>();
...
const logoPreviewUrl = computed(() => {
  ...
  if (typeof props.resolveResourceUrl === 'function') {
    return props.resolveResourceUrl(value);
  }
...
function beforeLogoUpload(file: UploadRawFile) {
  ...
  if (!(allowedType.includes(type) || allowedExtension.includes(extension))) {
    notifyError('Logo 仅支持 jpg/jpeg/png/webp/svg 格式');
    return false;
  }
...
async function uploadLogoRequest(options: ElUploadRequestOptions) {
  try {
    if (typeof props.uploadResource !== 'function') {
      throw new Error('未注入上传能力');
    }
  ...
  <el-form :model="props.formState" label-position="top" class="settings-form">
    <ObCard class="settings-card" title="基础布局">
      ...
      <el-form-item v-if="props.formState.shell.header.mode === 'customComponent'" label="独立组件">
        <el-select
          v-model="props.formState.shell.header.customComponentKey"
          class="w-320"
          placeholder="请选择组件"
        >
          <el-option
      ...
      <el-form-item label="背景色">
        <ObColorField v-model="props.formState.shell.header.tokens.bgColor" />
      </el-form-item>

      <el-form-item label="文字色">
        <ObColorField v-model="props.formState.shell.header.tokens.textColor" />
      </el-form-item>

      <el-form-item label="高度(px)">
        <el-input-number
          v-model="props.formState.shell.header.tokens.height"
          :min="40"
          :max="200"
        />
      </el-form-item>
      ...
      <el-form-item label="层级 z-index">
        <el-input-number
          v-model="props.formState.shell.header.tokens.zIndex"
          :min="1"
          :max="2000"
        />
      </el-form-item>
    ...
    <ObCard class="settings-card" title="品牌与标题">
      ...
      <el-form-item label="主标题字号(px)">
        <el-input-number
          v-model="props.formState.shell.header.behavior.titleFontSize"
          :min="12"
          :max="48"
        />
      </el-form-item>
      ...
      <el-form-item label="副标题字号(px)">
        <el-input-number
          v-model="props.formState.shell.header.behavior.subTitleFontSize"
          :min="10"
          :max="36"
        />
      </el-form-item>
      ...
      <el-form-item label="Logo 图片">
        <div class="logo-uploader">
          ...
          <div class="logo-tip">
            支持 jpg/jpeg/png/webp/svg，单张不超过 2MB。上传后自动写入 details。
          </div>
        </div>
      </el-form-item>
      ...
      <el-form-item label="Logo 宽度(px)">
        <el-input-number
          v-model="props.formState.shell.header.tokens.logoWidth"
          :min="40"
          :max="420"
        />
      </el-form-item>
      ...
      <el-form-item label="Logo 左偏移(px)">
        <el-input-number
          v-model="props.formState.shell.header.tokens.logoLeftMargin"
          :min="0"
          :max="200"
        />
      </el-form-item>
    ...
    <ObCard class="settings-card" title="导航模块">
      ...
      <el-form-item label="导航激活背景">
        <ObColorField v-model="props.formState.shell.header.tokens.activeBgColor" />
      </el-form-item>

      <el-form-item label="导航激活文字">
        <ObColorField v-model="props.formState.shell.header.tokens.activeTextColor" />
      </el-form-item>

      <el-form-item
        v-if="props.formState.shell.header.behavior.navSource === 'manual'"
        label="手工导航"
      >
        <div class="rows-panel">
          <div class="rows-header">
            <span>可配置名称、绑定页面（tabId）或外链 URL</span>
            <el-button type="primary" plain size="small" :icon="Plus" @click="addManualNavItem"
              >新增导航</el-button
            >
          </div>
          ...
          >
            ...
            >
              <el-option
                v-for="tab in props.pageTabOptions"
                :key="tab.value"
                :label="tab.label"
                :value="tab.value"
              />
    ...
    <ObCard class="settings-card" title="顶部公告模块">
      ...
      <el-form-item v-if="props.formState.shell.header.behavior.showTopNotice" label="公告背景色">
        <ObColorField v-model="props.formState.shell.header.tokens.noticeBgColor" />
      </el-form-item>

      <el-form-item v-if="props.formState.shell.header.behavior.showTopNotice" label="公告文字色">
        <ObColorField v-model="props.formState.shell.header.tokens.noticeTextColor" />
      </el-form-item>
    </ObCard>
    ...
    <ObCard class="settings-card" title="行动按钮与用户区">
      ...
      <el-form-item v-if="props.formState.shell.header.behavior.showActionButton" label="按钮文案">
        <el-input
          v-model="props.formState.shell.header.behavior.actionButtonText"
          maxlength="20"
          show-word-limit
        />
      </el-form-item>
      ...
      <el-form-item
        v-if="props.formState.shell.header.behavior.showActionButton"
        label="按钮跳转 URL"
      >
        <el-input
      ...
      <el-form-item
        ...
        label="按钮背景色"
      >
        <ObColorField v-model="props.formState.shell.header.tokens.actionBgColor" />
      </el-form-item>
      ...
      <el-form-item
        ...
        label="按钮文字色"
      >
        <ObColorField v-model="props.formState.shell.header.tokens.actionTextColor" />
      </el-form-item>
      ...
      <el-form-item
        ...
        label="按钮边框色"
      >
        <ObColorField v-model="props.formState.shell.header.tokens.actionBorderColor" />
      </el-form-item>
