...
export function isValidTag(tag: any): tag is TagItem {
  ...
  // 必须有 meta 对象
  if (!tag.meta || typeof tag.meta !== 'object') {
    return false;
  }

  // meta 必须有 title
  if (!tag.meta.title || typeof tag.meta.title !== 'string') {
    return false;
  }
...
export function validateTagConfig(config: any): boolean {
  ...
  // 验证必需字段
  if (!(config.path && validateTagPath(config.path))) {
    return false;
  }
  if (!config.meta || typeof config.meta !== 'object') {
    return false;
  }
  if (!config.meta.title || typeof config.meta.title !== 'string') {
    return false;
  }
...
export function isClosableTag(tag: TagItem): boolean {
  ...
  // 首页不可关闭
  if (tag.path === configManager.getHomePath()) {
    return false;
  }
