...
describe('ui auth entry source', () => {
  ...
  it('lite auth 入口只对 V2 保持异步壳，基础 LoginBox 留在同一 auth chunk', () => {
    ...
    expect(source).not.toContain("defineAsyncComponent(() => import('../components/auth/LoginBox.vue'))")
    expect(source).toContain("defineAsyncComponent(() => import('../components/auth/LoginBoxV2.vue'))")
  })
})
