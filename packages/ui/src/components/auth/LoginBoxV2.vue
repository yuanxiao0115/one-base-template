<script setup lang="ts">
  ...
  const props = withDefaults(
    defineProps<{
      ...
      encrypt?: boolean
      sm4KeyHex?: string
      loadCaptcha: (params: { captchaKey: string }) => Promise<CaptchaBizResponse<CaptchaBlockPuzzleData>>
      checkCaptcha: (params: { captcha: string; captchaKey: string }) => Promise<CaptchaBizResponse<unknown>>
    }>(),
    {
      title: '',
      usernameLabel: '账号',
