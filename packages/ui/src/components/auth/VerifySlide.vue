  ...
  props: {
    loadCaptcha: {
      type: Function as PropType<
        (params: { captchaKey: string }) => Promise<CaptchaBizResponse<CaptchaBlockPuzzleData>>
      >,
      required: true
    },
    checkCaptcha: {
      type: Function as PropType<
        (params: { captcha: string; captchaKey: string }) => Promise<CaptchaBizResponse<unknown>>
      >,
      required: true
    },
    sm4KeyHex: {
      ...
      if (this.status && this.isEnd === false) {
        ...
        if (this.barArea) {
          const max =
            ...
            Number.parseInt(String(Number.parseInt(this.blockSize.width, 10) / 2), 10) -
            2;
