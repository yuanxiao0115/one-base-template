...
const MENU_ICON_BASE_CLASSES = new Set([
  'iconfont',
  'iconfont-od',
  'dj-icons',
  'i-icon-menu',
...
function resolveBaseClass(iconClass: string): string | undefined {
  if (iconClass.startsWith('i-icon-')) {
    return 'i-icon-menu';
  }
  if (iconClass.startsWith('dj-icon-')) {
    return 'dj-icons';
  }
  if (iconClass.startsWith('pure-iconfont-')) {
    return 'pure-iconfont';
  }
...
const kind = computed<IconKind>(() => {
  ...
  // iconfont class
  if (
    raw.startsWith('i-icon-') ||
    raw.startsWith('icon-') ||
    raw.startsWith('dj-icon-') ||
    raw.startsWith('pure-iconfont-')
  ) {
    return 'class';
  }
