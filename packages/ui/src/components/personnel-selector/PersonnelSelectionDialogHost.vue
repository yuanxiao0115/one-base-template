...
function buildResult(): OpenPersonnelSelectionResult {
  ...
  const users = selectedItems.filter(
    (item): item is PersonnelSelectedUser => item.nodeType === 'user'
  );
  const orgs = selectedItems.filter(
    (item): item is PersonnelSelectedOrg => item.nodeType === 'org'
  );
  const roles = selectedItems.filter(
    (item): item is PersonnelSelectedRole => item.nodeType === 'role'
  );
  const positions = selectedItems.filter(
    (item): item is PersonnelSelectedPosition => item.nodeType === 'position'
  );
