...
const emit = defineEmits<{
  ...
  (event: 'search-clear' | 'search'): void;
...
function onEnterOrg(node: PersonnelOrgNode) {
  emit('enter-org', node);
}
...
<template>
  <div class="personnel-selector-source">
    <div class="personnel-selector-source__search">
      ...
      >
        <template #append>
          <el-button :loading="props.loading" :disabled="props.disabled" @click="onSearch"
            >搜索</el-button
          >
        </template>
      </el-input>
    ...
    >
      ...
      >
        <span
          class="personnel-selector-source__breadcrumb-title"
          @click="onBreadcrumbClick(index + 1)"
        >
          {{ item.title }}
        </span>
        <span
          v-if="index < breadcrumbItems.length - 1"
          class="personnel-selector-source__breadcrumb-separator"
        >
          /
        </span>
      </span>
        ...
        >
          <template v-if="isOrgNode(node)">
            ...
            <el-checkbox
              ...
              @update:model-value="onToggleOrg(node, Boolean($event))"
            >
              <span class="personnel-selector-source__org-text">
                {{ node.orgName || node.title }}
              </span>
            </el-checkbox>
            <span v-else class="personnel-selector-source__org-text" @click="onEnterOrg(node)">
              {{ node.orgName || node.title }}
            </span>
            <el-button
          ...
          <template v-else-if="isUserNode(node)">
            ...
            <el-checkbox
              ...
              @update:model-value="onToggleUser(node, Boolean($event))"
            >
