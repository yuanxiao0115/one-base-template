<script setup lang="ts">
  ...
  import { Comment, Fragment, Text, computed, defineComponent, type PropType, type VNode, useSlots } from 'vue';
  ...
  function isDeleteAction(node: VNode, label: string): boolean {
    ...
    if (String(props.type || '').toLowerCase() === 'danger') {
      return true;
    }
    if (normalizeClassName(props.class).toLowerCase().includes('danger')) {
      return true;
    }
  ...
  function resolveActionNodes(): ActionNode[] {
    const rawNodes = extractSlotNodes(slots.default?.() ?? []).filter((node) => node.type !== Text);
...
<template>
  <div class="ob-action-buttons" @click.stop>
    <VNodeRenderer v-for="action in visibleActions" :key="action.key" :node="action.vnode" />
