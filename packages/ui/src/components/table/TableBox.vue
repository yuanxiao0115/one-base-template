...
<template>
  <div class="one-table-bar">
    <div class="one-table-bar__head">
      <slot name="title"> <p class="one-table-bar__title">{{ title }}</p> </slot>
    </div>
    ...
    <div v-if="showSearchBar" class="one-table-bar__toolbar">
      <div class="one-table-bar__search">
        ...
        <div v-if="$slots.searchForm" class="one-table-bar__search-form"><slot name="searchForm" /></div>
      </div>

      <div v-if="$slots.buttons" class="one-table-bar__buttons"><slot name="buttons" /></div>
    </div>
    ...
    <div class="one-table-bar__content"><slot :size="size" :dynamic-columns="dynamicColumns" /></div>
  </div>

  <el-drawer v-model="drawerFlag" title="高级筛选" :size="400" append-to-body class="one-drawer">
    <div class="one-table-bar__drawer"><slot name="drawer" /></div>
    <template #footer>
