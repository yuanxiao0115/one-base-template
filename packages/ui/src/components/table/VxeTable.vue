<script setup lang="ts">
  ...
  function resolveColumnOverflow(column: TableColumn, index: number): boolean | string | undefined {
    ...
    if (isOperationColumn(column, index)) {
      return undefined;
    }
  ...
  function createRendererParams(column: TableColumn, params: VxeEventParams): TableColumnRendererParams {
  ...
  function convertColumns(columns: TableColumnList): Record<string, unknown>[] {
    return columns.map((column, index) => {
      ...
      mappedColumn.label = undefined;
      mappedColumn.prop = undefined;
      ...
      mappedColumn.showOverflowTooltip = undefined;
  ...
  function resolvePagerLayouts(
    ...
    const layouts = layout
      ...
      .filter((item): item is 'Total' | 'Sizes' | 'PrevPage' | 'Number' | 'NextPage' | 'FullJump' => Boolean(item));
